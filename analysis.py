...
def analyze_cosine_similarities(
    autoencoder_location="saes/sae.pkl", rectify=False, include_control=False
):
    """
    ...
    if include_control:
        ...
        control_cosine_similarities = probe_directions @ random_directions.transpose(
            0, 1
        )
        max_cosine_similarities_control = control_cosine_similarities.max(dim=1).values
        sns.kdeplot(
            max_cosine_similarities_control,
            label="Probe/Random",
            ax=ax2,
            color="orange",
        )
...
def analyze_cosine_similarities_3(rectify=False):
    ...
    cosine_similarities_masked_self = cosine_similarities - torch.eye(
        cosine_similarities.shape[0]
    )
    max_cosine_similarities_of_probes = cosine_similarities_masked_self.max(
        dim=1
    ).values
...
def get_probe_directions(probe_location="probes/probe_layer_6.pkl", rectify=False):
    ...
    if rectify:
        original_probe_directions = (
            1.5 * original_probe_directions - 0.5 * original_probe_directions.sum(dim=0)
        )
...
def get_autoencoder_directions(autoencoder_location="saes/sae_layer_6_trimmed.pkl"):
...
def evaluate_all_content_classification(
    directory="analysis_results",
    activations_file_name="all_activations.pkl",
    boards_file_name="all_boards.pkl",
):
    ...
    for i, feature_activation in tqdm(enumerate(all_activations.transpose(0, 1))):
        for j, board_position in enumerate(all_boards.transpose(0, 1)):
            for k, piece_class in enumerate([0, 1, 2]):
                ...
                metric = BinaryAUROC()
                metric.update(
                    feature_activation[ended_game_mask],
                    is_target_piece[ended_game_mask].int(),
                )
...
def evaluate_all_legal_moves_classification(
    activations_location="analysis_results/all_activations.pkl",
    legal_moves_location="analysis_results/all_legal_moves.pkl",
):
    ...
    for i, feature_activation in tqdm(enumerate(all_activations.transpose(0, 1))):
        for j, board_position in enumerate(all_legal_moves.transpose(0, 1)):
            for k, legality_class in enumerate([0, 1]):
                ...
                metric.update(
                    feature_activation[ended_game_mask],
                    is_target_legality[ended_game_mask].int(),
                )
                aurocs[i, j, k] = float(metric.compute())
                # print(f"F1 Score: {f1_score:.4f}")
...
def evaluate_all_probe_classification(
    activations_location="analysis_results/probe_activations.pkl",
    board_locations="analysis_results/probe_all_boards.pkl",
):
    with open(activations_location, "rb") as f:
    ...
    for board_position, all_class_logits in tqdm(enumerate(reshaped_activations)):
        ...
        for piece_class, this_class_odds in enumerate(all_class_odds):
            ...
            metric = BinaryAUROC()
            metric.update(
                this_class_odds[ended_game_mask], is_target_piece[ended_game_mask].int()
            )
...
def create_density_plot_contents(feature_number, board_position, separate_at_0=True):
    ...
    empty_own_enemy_scores = [
        scores[np.where(labels == class_number)] for class_number in range(3)
    ]
    ...
    if separate_at_0:
        ...
        empty_own_enemy_zero_scores = [
            class_scores[np.where(class_scores == 0)]
            for class_scores in empty_own_enemy_scores
        ]
        empty_own_enemy_zero_frequencies = [
            len(class_scores) / len(scores)
            for class_scores in empty_own_enemy_zero_scores
        ]
        empty_own_enemy_nonzero_scores = [
            class_scores[np.where(class_scores != 0)]
            for class_scores in empty_own_enemy_scores
        ]
        bottom = 0
        ...
        for class_number, class_name in enumerate(class_names):
            sns.kdeplot(
                empty_own_enemy_nonzero_scores[class_number],
                fill=True,
                label=class_name,
                ax=axs[1],
            )
            axs[0].bar(
                x=0,
                bottom=bottom,
                height=empty_own_enemy_zero_frequencies[class_number],
            )
    ...
    plt.title(
        f"Feature {feature_number} activations against Position {board_position} contents"
    )
    plt.legend()
...
def create_density_plot_legal(feature_number, board_position, separate_at_0=True):
    ...
    illegal_legal_scores = [
        scores[np.where(labels == class_number)] for class_number in range(2)
    ]
    ...
    if separate_at_0:
        ...
        for spine in axs[0].spines.values():
            spine.set_visible(False)
        illegal_legal_zero_scores = [
            class_scores[np.where(class_scores == 0)]
            for class_scores in illegal_legal_scores
        ]
        illegal_legal_zero_frequencies = [
            len(class_scores) / len(scores)
            for class_scores in illegal_legal_zero_scores
        ]
        illegal_legal_nonzero_scores = [
            class_scores[np.where(class_scores != 0)]
            for class_scores in illegal_legal_scores
        ]
        bottom = 0
        ...
        for class_number, class_name in enumerate(class_names):
            sns.kdeplot(
                illegal_legal_nonzero_scores[class_number],
                fill=True,
                label=class_name,
                ax=axs[1],
            )
            axs[0].bar(
                x=0, bottom=bottom, height=illegal_legal_zero_frequencies[class_number]
            )
    ...
    ax.set_xlabel("Feature Activation")
    plt.title(
        f"Feature {feature_number} activations against whether Position {board_position} is a legal move"
    )
    plt.legend()
...
def create_hist_for_probe(board_position, primary_class=0, separate_at_0=False):
    ...
    scores = 2 * all_activations[
        :, board_position, primary_class
    ] - 1 * all_activations[:, board_position, :].sum(dim=-1)
    ...
    empty, own, enemy = (
        scores[torch.where(this_position_labels == class_number)].detach().numpy()
        for class_number in class_names
    )
    fig, ax = plt.subplots()
    ax.hist(
        (enemy, empty, own),
        ...
        histtype="bar",
        stacked=True,
        label=["enemy", "empty", "own"],
    )
    ax.set_xlabel("Feature Activation")
    ax.set_ylabel("Frequency")
    ax.set_title(
        f"Position {board_position} probe, trying to separate class {class_names[primary_class]}"
    )
    plt.legend()
...
def create_triple_hist_for_probe(board_position, primary_class=0):
    ...
    for primary_class in range(3):
        ...
        scores = 1.5 * all_activations[
            :, board_position, primary_class
        ] - 0.5 * all_activations[:, board_position, :].sum(dim=-1)
        this_position_labels = all_boards[:, board_position]
        class_names = {0: "Empty", 1: "Own", 2: "Enemy"}
        empty, own, enemy = (
            scores[torch.where(this_position_labels == class_number)].detach().numpy()
    ...
    plt.tight_layout()
    save_file_name = (
        f"analysis_results/figures/triple_density_probe_pos_{board_position}.png"
    )
...
def find_top_aurocs_contents(k=10, directory="analysis_results"):
    ...
    print(
        "\n".join(
            [
                f"{top_locations_tuple}, {top_value:.4f}"
                for top_locations_tuple, top_value in zip(
                    top_locations_tuples, top_values
                )
            ]
        )
    )
...
def compare_legal_contents_aurocs(k=10, directory="analysis_results"):
    ...
    top_locations_tuples = [
        location_to_triple(x, legal_aurocs.shape) for x in top_locations
    ]
    these_content_aurocs = [
        contents_aurocs[(a, b, c - 1)] for a, b, c in top_locations_tuples
    ]
    print(
        "(Feature_number, board_position, class), Legal AUROC, Content AUROC, Difference"
    )
    print(
        "\n".join(
            [
                f"{top_locations_tuple}, {legal_auroc:.4f}, {content_auroc:.4f}, {content_auroc-legal_auroc:.4f}"
                for top_locations_tuple, legal_auroc, content_auroc in zip(
                    top_locations_tuples, these_legal_aurocs, these_content_aurocs
                )
            ]
        )
    )
...
def show_top_activating(
    ...
    """
    randomly select top_k activation board from top_n activations

    if legal_status is None, then it will select from all activations, otherwise it will select from only legal or illegal activations
    """
    ...
    if legal_status is not None:
        ...
        if legal_status:
            all_boards = all_boards[
                torch.where(all_legal_moves[:, marked_position] == 1)
            ]
            all_activations = all_activations[
                torch.where(all_legal_moves[:, marked_position] == 1)
            ]
            # junky code
            selected_legal_moves = all_legal_moves[
                torch.where(all_legal_moves[:, marked_position] == 1)
            ]
        else:
            all_boards = all_boards[
                torch.where(all_legal_moves[:, marked_position] == 0)
            ]
            all_activations = all_activations[
                torch.where(all_legal_moves[:, marked_position] == 0)
            ]
    ...
    num_data = filtered_boards.shape[0]
    these_activations = all_activations[:, feature_number]
    filtered_activations = these_activations[torch.where(all_boards[:, 0] > -100)]

    # randomly select top k from top n
    # Step 1: Get the top n activations
    top_n_activations, top_n_indices = torch.topk(filtered_activations, k=top_n)

    # Step 2: Randomly select k from the top n
    random_indices = torch.randperm(top_n)[:top_k]

    # Get the k random activations and their corresponding indices from the top n
    best_activations = top_n_activations[random_indices]
    best_indices = top_n_indices[random_indices]
    ...
    fig, axs = plt.subplots(
        2, top_k, figsize=(10, 5)
    )  # Specify subplot grid and figure size directly
    plt.suptitle(
        f"Feature {feature_number}, with Position {marked_position} marked, indices: {best_indices}"
    )
...
def plot_board(ax, board, activation, marked_position=-1):
    ...
    if black_pieces_locations:
        ax.scatter(
            x=torch.tensor(black_pieces_locations)[:, 0],
            y=torch.tensor(black_pieces_locations)[:, 1],
            c="black",
            s=marker_size,
            linewidths=1.5,
            edgecolors="black",
    ...
    if white_pieces_locations:
        ax.scatter(
            x=torch.tensor(white_pieces_locations)[:, 0],
            y=torch.tensor(white_pieces_locations)[:, 1],
            c="white",
            s=marker_size,
            linewidths=1.5,
            edgecolors="black",
...
def save_activations_boards_and_legal_moves(
    sae_location="saes/sae_layer_6.pkl",
    eval_dataset_type="probe_test",
    offset=0,
    ...
    test_dataloader = iter(
        get_dataloader(
            eval_dataset_type, window_length=sae.window_length, batch_size=10
        )
    )
    activations = []
    boards = []
    ...
    for test_input, test_labels in tqdm(test_dataloader):
        (
            ...
            sparsity_loss,
            normalized_logits,
        ), total_loss = sae(test_input, None)
        activations.append(hidden_layer)
        boards.append(
            sae.trim_to_window(test_labels, offset=offset)
        )  # index n is the board state after before move n+start_window_length
        legal_moves.append(
            sae.trim_to_window(history_to_legal_moves(test_input.cpu()), offset=offset)
        )  # index in are the legal moves on turn n+1+start_window_length
    all_activations = torch.cat(activations).flatten(
        end_dim=-2
    )  # shape (dw,f), where d= dataset size (2000), w=trimmed window length (52), f=num_features (1024)
    all_boards = torch.cat(boards).flatten(
        end_dim=-2
    )  # shape (dw,b), where d= dataset size (2000), w=trimmed window length (52), b=board_size(64)
    all_legal_moves = torch.cat(legal_moves).flatten(
        end_dim=-2
    )  # shape (dw,b), where d= dataset size (2000), w=trimmed window length (52), b=board_size(64)
...
def analyse_saes_across_seed_range():
    for idx in range(1, 11):
        ...
        save_activations_boards_and_legal_moves(
            sae_location=autoencoder_location, save_directory=dir, offset=1
        )
...
def compare_top_features_across_seeds():
    ...
    for seed in range(1, 11):
        ...
        all_top_locations.extend(
            [
                position
                for (feature, position, piece_type), auroc in zip(
                    top_locations_tuples, top_values
                )
                if auroc >= 0.9
            ]
    ...
    print(
        "\n".join(
            f"{pos}: {freq}"
            for pos, freq in sorted(counts.items(), key=lambda x: x[1], reverse=True)
        )
    )
    ...
    for i in range(8):
        for j in range(8):
            text = ax.text(
                j, i, int(data_to_plot[i, j]), ha="center", va="center", color="w"
            )
...
def graph_MCS_and_AUROC_correlation(autoencoder_location="saes/sae.pkl", rectify=False):
    ...
    for probe_number, feature_number in enumerate(locations):
        corresponding_aurocs[probe_number] = contents_aurocs[
            feature_number, probe_number % 64, probe_number // 64
        ]
